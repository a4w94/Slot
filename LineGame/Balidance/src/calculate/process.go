...
func (result *MainGameEachRoundResult) MainGameSpecila() {
	//wildmultiple:=make(map[int]int)
	wildmultiple := map[int]int{
		0: 1,
		1: 2,
		2: 3,
		3: 4,
	...
	multiple := wildmultiple[panelwildamout]

	for index, m := range result.LineGameComboResult {
		var wildamount int
		for i := 0; i < m.Combo; i++ {
			if m.ResultLine[i] == info.Wild {
				wildamount++
			}
		}
		if wildamount > 0 {
			result.LineGameComboResult[index].Score *= multiple
			result.LineGameComboResult[index].Multiple = multiple
			if multiple != checkmultiple {
				result.LineGameComboResult[index] = scoretools.LineGameEachWay{}
			}
			//result.LineGameComboResult[index] = scoretools.LineGameEachWay{}
