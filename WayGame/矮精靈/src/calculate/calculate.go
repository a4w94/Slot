...
func (result *TotalRoundResult) TotalRound() {
	...
	for i := 0; i < Session; i++ {
		...
		if info.GameMode == info.GameStatus.WayGame {
			...
			result.SymbolComboTotalHit.SymbolScore_WayGame(info.GameStatus.MainGame, each_Round_Result.MainGame.Way_Game_Combo, each_Round_Result.MainGame.ScatterResult)
