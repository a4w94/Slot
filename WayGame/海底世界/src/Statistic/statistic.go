...
func (result *StatisticTable) SymbolScore_WayGame(game_status string, input scoretools.Way_Game_Combo, inputScatter scoretools.ScatterResult, inputBonus scoretools.Bonus) {
	if game_status == info.GameStatus.MainGame {
		for _, tmp := range input.WayGameComboResult {
		...
		result.NGScore[info.Bonus][inputBonus.Combo] += inputBonus.Score
